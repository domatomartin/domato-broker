...
function parseHtml(html: string): ParseResult {
  ...
  // ── Cuenta (TABLE 2, fila con "Cuenta") ───────────────────────────────
  ...
  for (const tr of Array.from(tables[2]?.querySelectorAll("tr") ?? [])) {
    ...
    if (cells[0]?.includes("Cuenta")) {
      cuenta = cells[1]?.replace(/[^0-9]/g, "") || null;
      break;
    }
  }

  // ── TABLE 4: Posición de Efectivo y Valores en Custodia ───────────────
  // Columns: [nombre, fecha_mov, moneda_code, saldo, notas?]
  // Algunos bonos aparecen dos veces (regular + Certif.BVM_) → sumar valor_nominal
  const posMap = new Map<
    string,
    { moneda: string; valor_nominal: number; fecha_compra: string | null }
  >();
  ...
  // ── TABLE 8: Detalle de cupones e intereses ────────────────────────────
  // Columns: [nombre+ISIN, nominal, vto, freq, prox_pago, tasa, int_corrido]
  ...
  // ── Merge positions + coupon data ──────────────────────────────────────
  const bonds: ParsedBond[] = [];
  for (const [nombre, pos] of posMap.entries()) {
    const cup = cupMap.get(nombre) ?? {
      isin: null,
      cupon: null,
      proximo_vencimiento: null,
      proximo_pago_interes: null,
      interes_corrido: 0,
    };
...
export default function HtmlImport({
  ...
  async function handleConfirm() {
    ...
    for (const bond of bonds) {
      ...
      if (bond.isin) {
        query = query.eq("isin", bond.isin);
        if (cuenta) query = query.eq("cuenta", cuenta);
      } else {
        query = query.eq("nombre", bond.nombre);
        if (cuenta) query = query.eq("cuenta", cuenta);
      }
      ...
      if (err) {
        setError(`Error actualizando "${bond.nombre}": ${err.message}`);
        setLoading(false);
        return;
      }

      if (!data || data.length === 0) {
        noEncontrados.push(bond.nombre);
      } else {
        updated++;
      }
    }
    ...
    if (noEncontrados.length > 0) {
      msg += ` No encontrados en cartera (${noEncontrados.length}): ${noEncontrados
        .slice(0, 3)
        .join(", ")}${noEncontrados.length > 3 ? "…" : ""}.`;
    }
    setStatus(msg);
    onImported?.();
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-3">
        ...
        {result?.cuenta && (
          <span className="text-xs text-muted">
            Cuenta:{" "}
            <span className="text-paper font-medium">{result.cuenta}</span>
          </span>
        )}
      </div>
      ...
      {loading && (
        <p className="text-xs text-muted animate-pulse">Procesando…</p>
      )}
        ...
        <div className="flex flex-col gap-3 mt-1">
          <p className="text-xs text-muted">
            ...
            <span className="text-paper font-semibold">
              {result.bonds.length} activos
            </span>{" "}
            en el informe. Revisá los datos antes de confirmar:
          </p>
            ...
            <table className="w-full text-xs border-collapse">
              ...
              <tbody>
                {result.bonds.map((b, i) => (
                  <tr
                    key={i}
                    className="border-b border-ink-border/20 hover:bg-ink-border/10"
                  >
                    <td className="px-2 py-1 text-paper max-w-[180px] truncate">
                      {b.nombre}
                    </td>
                    <td className="px-2 py-1 text-muted font-mono text-[10px]">
                      {b.isin ?? "—"}
                    </td>
                    <td className="px-2 py-1 text-right tabular-nums">
                      {b.valor_nominal.toLocaleString("es-UY", {
                        maximumFractionDigits: 2,
                      })}
                    </td>
                    <td className="px-2 py-1 text-muted">{b.moneda}</td>
                    <td className="px-2 py-1 text-right tabular-nums">
                      {b.cupon != null ? `${b.cupon.toFixed(4)}%` : "—"}
                    </td>
                    <td className="px-2 py-1 text-muted">
                      {b.proximo_pago_interes ?? "—"}
                    </td>
                    <td className="px-2 py-1 text-muted">
